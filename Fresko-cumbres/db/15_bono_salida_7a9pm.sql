-- El bono de puntualidad de la SALIDA se gana marcando entre hora_fin
-- (7:00 pm) y hora_fin + tolerancia_bono_salida_min (9:00 pm por defecto).
-- Antes cualquier salida marcada después de las 7:00 pm contaba para el
-- bono sin límite superior; ahora se sigue registrando siempre (para el
-- pago del día), pero solo cuenta para el bono si es dentro de esa
-- ventana, igual que ya pasa con la entrada.

alter table public.parametros
  add column if not exists tolerancia_bono_salida_min integer;

update public.parametros
  set tolerancia_bono_salida_min = 120
  where tolerancia_bono_salida_min is null;
...
declare
  ...
  v_nombre text; v_equipo text; v_abre time; v_cierra_bono time; v_a_tiempo boolean;
begin
  ...
  if v_tipo = 'entrada' then
    ...
    v_cierra_bono := v_par.hora_inicio + (v_par.tolerancia_entrada_min || ' minutes')::interval;
    ...
    v_a_tiempo := v_hora <= v_cierra_bono;
  else
    v_abre        := v_par.hora_fin - (coalesce(v_par.tolerancia_salida_min,0) || ' minutes')::interval;
    v_cierra_bono := v_par.hora_fin + (coalesce(v_par.tolerancia_bono_salida_min,0) || ' minutes')::interval;
    if v_hora < v_abre then
  ...
  return jsonb_build_object('ok', true, 'folio', v_f, 'nombre', v_nombre, 'tipo', v_tipo,
    ...
    'mensaje', case
      when not v_a_tiempo then
        'Quedó registrada tu ' || v_tipo || ', pero fuera de la hora del bono de puntualidad.'
      else
...
begin
  ...
  return jsonb_build_object(
    ...
    'cierra_salida', to_char(v_par.hora_fin + (coalesce(v_par.tolerancia_bono_salida_min,0) || ' minutes')::interval, 'HH12:MI am'),
